...
* **Upcoming release — timezone settings ([#86](https://github.com/sgoudelis/ground-station/issues/86)):** Setup detects the station timezone from its coordinates and lets you choose a different timezone for your account. Additional accounts start with the station's geographic timezone. Existing saved preferences are preserved: if your clock is incorrect after upgrading, open **account menu → Preferences → Regional & Language → Timezone**, choose your timezone (for example, `Europe/Amsterdam`), and save. Daylight saving time is handled automatically; changing the Docker or server timezone does not change your account preference.
